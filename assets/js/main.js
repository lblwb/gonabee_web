  ...
  const observeSelection = (thumbnailSlider, mainSlider) => {
    ...
    const checkAndSlide = () => {
      const selected = container.querySelector(
        '.cardImagesThumbItem[data-select="true"]'
      );
      if (selected) {
        const realIndex = parseInt(
          selected.getAttribute("data-swiper-slide-index"),
          10
        );
        if (!isNaN(realIndex)) {
          console.log(
            '➡️ Переход к слайду с data-select="true", индекс:',
            realIndex
          );
    ...
    window.states.singlePrdCard = Vue.createApp({
      setup() {
        ...
        const selectCartColor = (color) => {
          ...
          //
          window.states.shopPrdDetailCardImages.selectSlideImgByColorQuery(
            color
          );
        };
