...
const bannerMainHomeSlider = async () => {
    ...
    const paginationContainer = document.querySelector(".bannerMainHomeSlider .bottomPaginateCorn");
...
const bannerProductSlider = async () => {
    ...
    const paginationContainer = document.querySelector(".mainMobSlider .bottomPaginate");
...
const mobMainProductsSlider = async () => {
    try {
        const mobMainProductSliders = document.querySelector(".bannerMainHome .bannerMainHomeSlider");
...
const ideaProductsSlider = async () => {
    ...
    ideaProductSliderBlockList.forEach((SliderEl, index) => {
        ...
        var ideaProductSlider = new Swiper(SliderEl, {
            ...
            pagination: {
                el: document.querySelector('.ideaProduct .ideaProductBody .ideaBottomPaginate'),
                clickable: true,
                renderBullet: function (index, className) {
                    return `<span class="${className} bullet"></span>`;
                },
    ...
    const singlePrdCard = () => {
        ...
        window.states.singlePrdCard = Vue.createApp({
            setup() {
                ...
                const getProductsCartExist = (ProductId) => Vue.computed(() => appShop.value.products.includes(ProductId));
                ...
                const getSelectedWhtLst = (productId) => Vue.computed(() => !(window.states.headMainNav.getFavorites().includes(productId))).value;
                ...
                const addToCartBtn = (event, productItem) => {
                    ...
                    if (!appShop.value.cart.btnAddActive) {
                        ...
                        if (event.target.querySelector('.addToCardBtnHeadingTitle')) {
                            event.target.querySelector('.addToCardBtnHeadingTitle').innerHTML = 'Перейти в корзину';
                        }
                    } else {
                        // Переход в корзину
            ...
            setup() {
                ...
                const addToCartMob = (event, productItem) => {
                    ...
                    if (!appMobShop.value.cart.btnAddActive) {
                        ...
                        showAddToCartNotification(productId);
                        console.log(event.target);
                        if (document.querySelector('#singlePrdMob .addToCartBtn .addToCartBtnTitle')) {
                            document.querySelector('#singlePrdMob .addToCartBtn .addToCartBtnTitle').innerHTML = 'Перейти в корзину';
                        }
                    } else {
                        // Переход в корзину
                        window.location.href = '/cart'
                ...
                const toggleMobSizePanel = () => {
                    appMobShop.value.cart.mob.size.show = !appMobShop.value.cart.mob.size.show;
                };
    ...
    const headMainNav = () => {
        ...
        window.states.headMainNav = Vue.createApp({
            setup() {
                ...
                const toggleMobNav = () => {
                    appMainNav.value.mob.nav_menu.show = !appMainNav.value.mob.nav_menu.show;
                };

                const toggleMobSearch = () => {
                    appMainNav.value.mob.nav_search.show = !appMainNav.value.mob.nav_search.show;
                };
                ...
                const searchText = async () => {
                    const query = appMainNav.value.mob.nav_search.context.queryText.trim();
                    ...
                    try {
                        const response = await fetch(
                            `/wp-admin/admin-ajax.php?action=search_prd_or_cat&query=${encodeURIComponent(query)}`
                        );
                        const data = await response.json();
                        appMainNav.value.mob.nav_search.products.result.list = data.products;
                        appMainNav.value.mob.nav_search.products.count = data.products ? data.products.length : 0;
                        //
                        appMainNav.value.mob.nav_search.cat.result.list = data.categories;
                        appMainNav.value.mob.nav_search.cat.count = data.categories ? data.categories.length : 0;
                    } catch (error) {
                ...
                // Следим за изменением состояния меню и блокируем body
                Vue.watch(
                    () => appMainNav.value.mob.nav_menu.show || appMainNav.value.mob.nav_search.show,
                    (show) => {
                        if (show) {
                            document.body.style.overflow = 'hidden';     // блокируем прокрутку
                            document.body.style.touchAction = 'none';   // предотвращаем скролл на мобильных
                ...
                const checkAndUpdateLogo = () => {
                    ...
                    if (window.scrollY || document.documentElement.scrollTop || window.innerWidth <= 768) {
                        ...
                        if (isMobile) {
                            if (scrollTop < 70 && !appMainNav.value.mob.nav_menu.logo.show) {
                                appMainNav.value.mob.nav_menu.logo.show = true;
                            } else if (scrollTop >= 70 && appMainNav.value.mob.nav_menu.logo.show) {
        ...
        window.states.headMainMbFilter = Vue.createApp({
            setup() {
                ...
                const toggleMobFilter = () => {
                    appMainFilter.value.mob.nav_filter.show = !appMainFilter.value.mob.nav_filter.show;
                    console.log("mob-filter-show", appMainFilter.value.mob.nav_filter.show);
                };
        ...
        window.states.shopPrdDetailCardImages = Vue.createApp({
            setup() {
                ...
                const appShopDetailCardSlider = Vue.ref({
                    ...
                    select: {
                        slide: {
                            src: window.prdImagesSlides && window.prdImagesSlides.length > 0 ? window.prdImagesSlides[0] : null
                        }
                    }
                });
            ...
            setup() {
                ...
                const applyPromocode = async () => {
                    ...
                    try {
                        ...
                        if (response.data.success) {
                            appPromocodeStates.message = response.data.message || 'Промокод успешно применен!';
                            ...
                            appPromocodeStates.message = response.data.message || 'Ошибка применения промокода';
                        ...
                        console.error('Promocode application failed:', error);
                        appPromocodeStates.message = 'Произошла ошибка при обработке запроса';
                        appPromocodeStates.isError = true;
                    } finally {
                ...
                const updateTotalCount = (count = 0) => {
                    try {
                        const cartTotalAmount = document.querySelectorAll('#cartTotalCount');
                ...
                const updateTotalEditAmount = (amount = 0) => {
                    try {
                        const cartTotalAmount = document.querySelectorAll('#cartTotalAmount');
                        cartTotalAmount.forEach((el) => {
                            el.innerHTML = amount;
                        })
                    } catch (e) {
                    ...
                    axios.post(ajaxUrl, {
                        ...
                        .then(res => {
                            if (res.data.success) {
                                ...
                                const qtyElement = document.querySelector(`[data-cart-key-id="${cartItemKey}"]`);
                                if (qtyElement) {
                        ...
                        .then(res => {
                            if (res.data.success) {
                                ...
                                if (res.data.data.action === 'removed') {
                                    // Также можно обновить DOM напрямую
                                    const prdElement = document.querySelector(`[data-prd-key-id="${cartItemKey}"]`);
                ...
                Vue.onMounted(() => {
                    ...
                    if (appCartStates.value.cart.list && appCartStates.value.cart.list.length) {
    ...
    const ctgFilterPc = () => {
        ...
        elAppMainFilters.forEach((elAppMainFilter) => {
            const appFilter = Vue.createApp({
                data() {
                    return {
                        expandedSections: ['subcategory', 'categories', 'promotions', 'price', 'sizes', 'colors', 'collections', 'occupation', 'toggles'],
                        selectedCategories: [],
                ...
                mounted() {
                    ...
                    this.selectedSubcategory = params.get('subcategory') ? params.get('subcategory').split(',') : [];
                    this.promotionActive = params.get('promotion') === '1';
                    this.minPrice = parseFloat(params.get('min_price')) || window.filterData.minPrice;
                    this.maxPrice = parseFloat(params.get('max_price')) || window.filterData.maxPrice;
                    this.selectedSize = params.get('size') || '';
                    this.selectedColors = params.get('color') ? params.get('color').split(',') : [];
                    this.selectedCollections = params.get('collection') ? params.get('collection').split(',') : [];
                    this.selectedOccupations = params.get('occupation') ? params.get('occupation').split(',') : [];
                    this.onSale = params.get('on_sale') === '1';
                    this.newCollection = params.get('is_new') === '1';
                    this.trending = params.get('is_trending') === '1';
                    ...
                    setTimeout(() => {
                        ...
                        noUiSlider.create(slider, {
                            ...
                            connect: true,
                            range: {'min': window.filterData.minPrice, 'max': window.filterData.maxPrice}
                        });
                        //
                ...
                methods: {
                    ...
                    updateUrlHistory(params) {
                        ...
                        // вызываем AJAX или обновляем DOM вручную
                        this.fetchProductsWithFilters(params);
                    },
                    applyFilters() {
                        const params = new URLSearchParams();
                        if (this.selectedSubcategory.length) params.set('subcategory', this.selectedSubcategory.join(','));
                        if (this.promotionActive) params.set('promotion', '1');
                        if (this.minPrice > window.filterData.minPrice) params.set('min_price', this.minPrice);
                        if (this.maxPrice < window.filterData.maxPrice) params.set('max_price', this.maxPrice);
                        ...
                        if (this.selectedOccupations.length) params.set('occupation', this.selectedOccupations.join(','));
                        if (this.onSale) params.set('on_sale', '1');
                        if (this.newCollection) params.set('is_new', '1');
                        if (this.trending) params.set('is_trending', '1');
                        // window.location.href = '?' + params.toString();
                        this.updateUrlHistory(params)
                    }
                },
...
const hideLogoMob = () => {
    ...
    const showLogo = () => {
        gsap.fromTo(headerMbBarNavLogo,
            {
                height: 0,
                paddingTop: 0,
                paddingBottom: 0,
