...
class ClaimService
{
    ...
    public function __construct(
        ClaimLinkRepository $claimLinkRepository, Claims $claims, GeoCoderInterface $geoCoder
    )
    {
    ...
    public function calculateShippingPrice(
        string $key, RoutePoint $source, array $orders, ?string $tariff, array $clientRequirements, ?DateTime $due = null, $waitUntilReadyForApproval = true
    ): Claim
    {
        ...
        if (is_null($claimLink)) {
            ...
            return $waitUntilReadyForApproval ? $this->claims->getUntilReadyForApproval($claim->getId(), $recall = true) : $this->claims->get($claim->getId());
        }

        $currentMetaHash = $this->claimLinkMetaHash->generate($items, $source, $destinations, $tariff, $clientRequirements);

        if (!$claimLink->metaWasUpdated($currentMetaHash)) {
            return $waitUntilReadyForApproval ? $this->claims->getUntilReadyForApproval($claimLink->getClaimId()) : $this->claims->get($claimLink->getClaimId());
        }
        ...
        file_put_contents(YGO_PLUGIN_DIR . '/debug.txt', json_encode($claimLink));

        return $waitUntilReadyForApproval ? $this->claims->getUntilReadyForApproval($claim->getId(), $recall = true) : $this->claims->get($claimLink->getClaimId());
    }
    ...
    */
    ...
    {
        $claimLink = $this->claimLinkRepository->get($key);
        file_put_contents(YGO_PLUGIN_DIR . '/debug_key.txt', $key);
        if (is_null($claimLink)) {
            throw new ClaimNotFoundException('Claim link not found in repository');
        }
    ...
    private function createClaim(
        string $key, array $items, RoutePoint $source, array $destinations, ?string $tariff = null, array $clientRequirements = [], ?DateTime $due = null
    ): Claim
    {
        $claim = $this->prepareClaim($items, $source, $destinations, $tariff, $clientRequirements, $due);
        ...
        if (count($destinations) === 1) {
            ...
            $link = new ClaimLink($key, $currentMetaHash, $destination->getAddress()->getFullName(), $destination->getAddress()->getLat(), $destination->getAddress()->getLon(), $claim->getId());
        } else {
            $link = new ClaimLink($key, $currentMetaHash, 'multi point address', null, null, $claim->getId());
        }

        $this->claimLinkRepository->store($link);
        file_put_contents(YGO_PLUGIN_DIR . '/debug1.txt', $key . json_encode($link));
    ...
    private function prepareClaim(
        array $items, RoutePoint $source, array $destinations, ?string $tariff = null, array $clientRequirements = [], ?DateTime $due = null
    ): Claim
    {
    ...
    private function checkIsFull(
        array $items, RoutePoint $source, array $destinations
    ): bool
    {
        $emptyFields = [];
