...
class Claims extends Resource
{
    ...
    {
        $claim_data = $this->claimToArray($claim);
    ...
    {
        $this->call('claims/accept', Client::API_V2, [
            'query' => ['claim_id' => $id],
            'json' => [
                'version' => $version,
            ],
    ...
    {
        $this->call('claims/cancel', Client::API_V2, [
            'json' => [
                'version' => $version,
    ...
    private function mapClaim(array $raw): Claim
    {
        ...
        foreach ($raw['items'] as $rawItem) {
            ...
            $item = new ClaimItem($id, $extraId, $dropOffPointOrderIdMap[$rawItem['droppof_point']] ?? null, $rawItem['title'], new Size($rawItem['size']['width'], $rawItem['size']['length'], $rawItem['size']['height']), new Money($rawItem['cost_value'], $rawItem['cost_currency']), $rawItem['weight'], $rawItem['quantity']);
        ...
        $claim->setAvailableCancelStatus(isset($raw['available_cancel_state']) ? AvailableCancelStatus::fromCode($raw['available_cancel_state']) : null);

        if (isset($raw['performer_info'])) {
            $driver = new Driver($raw['performer_info']['courier_name'], $raw['performer_info']['car_model'], $raw['performer_info']['car_number']);
    ...
    {
        $address = new Address($rawPoint['address']['fullname'], $rawPoint['address']['coordinates'][1], $rawPoint['address']['coordinates'][0], $rawPoint['address']['comment'] ?? '');
        ...
        $point = new RoutePoint(new Contact($rawPoint['contact']['name'], $rawPoint['contact']['phone'], $rawPoint['contact']['email'] ?? ''), $address, !$rawPoint['skip_confirmation'], // send == !skip
    ...
    private function checkHasNoError(array $raw): void
    {
        if (isset($raw['error_messages'])) {
            if ($raw['error_messages'][0]['code'] === 'errors.default_performer_not_found' || empty($raw['error_messages'])) {
                return;
            }
    ...
    private function claimToArray(Claim $claim): array
    {
        ...
        foreach ($claim->getItems() as $item) {
            ...
            $itemArray = [
                ...
                'cost_value' => number_format($item->getCost()->getValue(), 2, '.', ''),
                'cost_currency' => $item->getCost()->getCurrency(),
                'weight' => $item->getWeight(),
                'quantity' => $item->getQuantity(),
                'fiscalization' => [
                    'vat_code_str' => $settings['vat'],
                    'supplier_inn' => $settings['inn'],
                    'article' => $extra_id,
                ],
            ];
            $items[] = $itemArray;
        ...
        if ($settings['post_payment'] == 'yes' && in_array($payment_method, ['ygo_cash', 'ygo_card'])) {
            //добавляем доставку
            $delivery_item_array = [
                'title' => __('Доставка', 'yandex-go-delivery'),
                'pickup_point' => $claim->getSource()->getId(),
                'droppof_point' => $delivery_item->getDropOffPointId(),
                'cost_value' => number_format($order->get_shipping_total(), 2, '.', ''),
                'cost_currency' => $delivery_item->getCost()->getCurrency(),
                'quantity' => 1,
                'size' => [
                    'length' => $delivery_item->getSize()->getLength(),
                    'width' => $delivery_item->getSize()->getWidth(),
                    'height' => $delivery_item->getSize()->getHeight(),
                ],
                'weight' => $delivery_item->getWeight(),
                'fiscalization' => [
                    'vat_code_str' => $settings['vat'],
                    'supplier_inn' => $settings['inn'],
                    'article' => '11111111',
        ...
        if ($settings['post_payment'] == 'yes' && in_array($payment_method, ['ygo_cash', 'ygo_card'])) {
            ...
            $destinations = $claim->getDestinations();

            foreach ($routePoints as $key => $route_point) {
                $routePoints[$key]['payment_on_delivery'] = [
                    "payment_method" => $payment,
                    "customer" => [
                        "email" => $destinations[0]->getContact()->getEmail(),
                        "phone" => $destinations[0]->getContact()->getPhone(),
                    ],
                ];
            }
        }
