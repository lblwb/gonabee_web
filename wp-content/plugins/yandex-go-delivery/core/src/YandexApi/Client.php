...
class Client {
	...
	public function __construct(
		HttpClient $httpClient, string $token, bool $userProdApi = true, string $referralSourceName = '', string $acceptLanguage = 'ru_RU'
	) {
		$this->token              = $token;
	...
	public function call( string $base, string $path, int $apiVersion, array $options ) {
		$response = $this->httpClient->sendPost( $this->buildUrl( $base, $apiVersion, $path, $options['query'] ?? [] ), $options['json'] ?? [], [
		...
		if ( $response->getCode() !== 200 ) {
			if ( isset( $result['code'] ) && 'payment_on_delivery_disabled' === $result['code'] ) {
				$result['message'] = __( 'The post-payment is not enabled for your account', 'yandex-go-delivery' );
			}
